  ...
  el "header" $ do
    ...
    rec let newValueEntered = ffilter (==keycodeEnter) (_textInput_keypress descriptionBox)
        ...
        descriptionBox <- textInput $ def & setValue .~ fmap (const "") newValueEntered
                                          & attributes .~ constDyn (mconcat [ "class" =: "new-todo" , "placeholder" =: "What needs to be done?" , "name" =: "newTodo"])
...
taskList activeFilter tasks = do
  elAttr "section" ("class" =: "main") $ do
    -- Create "toggle all" button
    rec let combineItemChanges = fmap (foldl' (.) id) . mergeList . map (\(k, v) -> fmap (flip Map.update k) v) . Map.toList
    ...
    return ( mergeWith (.) [ itemChanges , toggleAllChanges]
          , Map.size <$> updated items)


todoItemCheckbox :: MonadWidget t m
         => Dynamic t Bool
         -> Dynamic t String
         -> m (Event t Bool, Event t (), Event t ())
todoItemCheckbox completed description = do
  elAttr "div" ("class" =: "view") $ do
    rec let setCompleted = fmap not $ tag (current completed) completedCheckbox
...
todoItemEditor :: MonadWidget t m
         => Dynamic t Bool
         -> Dynamic t (Map String String)
         -> Dynamic t String
         -> m (Dynamic t Bool, Event t (Task -> Maybe Task))
todoItemEditor completed attrs description = do
  -- Set the current value of the editBox whenever we start editing (it's not visible in non-editing mode)
  -- Set the todo item's description when the user leaves the textbox or presses enter in itp

  elDynAttr "li" attrs $ do
      rec let setEditValue = tagDyn description $ ffilter id $ updated editing
              ...
              changeSelf = mergeWith (>=>) [ fmap (\c t -> Just $ t { taskCompleted = c }) setCompleted
                                            ...
                                            , fmap (\d t -> case T.unpack $ T.strip $ T.pack d of
                                                      { "" -> Nothing ; trimmed -> Just $ t { taskDescription = trimmed } }) setDescription
                                            ]
          ...
          -- Without the delay, the focus doesn't take effect because the element hasn't become unhidden yet;
          -- we need to use postGui to ensure that this is threadsafe when built with GTK
          postGui <- askPostGui
          performEvent_ $ fmap (const $ liftIO $ void $ forkIO $ threadDelay 1000 >> postGui (liftIO $ focus $ _textInput_element editBox)) startEditing

          editBox <- textInput $ def & setValue .~ setEditValue & attributes .~ constDyn ("class" =: "edit" <> "name" =: "title")
          (setCompleted, destroy, startEditing) <- todoItemCheckbox completed description
...
todoItem todo = do
  rec description <- liftM nubDyn $ mapDyn taskDescription todo
      ...
      (editing', changeTodo) <- todoItemEditor completed attrs description
